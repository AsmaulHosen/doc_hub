...
<header>
    <script>
        function MySuccessFn() {
            swal({
                    title: "Medicine Details Add successfully",
                    text: "",
                    type: "success",
                    timer: 1500,
...
if ($_POST) {
    ...
    if (number_format($unit_price) >= number_format($sell_price)) {
        ...
        $check = "SELECT * FROM medicine_details WHERE vendor_id = '$vendor_id' and medicine_name = '$medicine_name'";
        $rs = mysqli_query($con, $check);
        $data = mysqli_fetch_array($rs, MYSQLI_NUM);
        if (isset($data)) {
            echo "<script type= 'text/javascript'>MyCheckFn();</script>";
        } else {
            $sql = "INSERT INTO medicine_details(medicine_name,categories_medication,categories_drug,type,unit_price,sell_price,age,qty,vendor_id) VALUES ('$medicine_name','$categories_medication','$categories_drug','$type','$unit_price','$sell_price','$age','$qty','$vendor_id');";
